...
var photoInput = document.querySelector ( "#photoWeb" )
...
function registerUser () {
	fetch ( 'http://localhost:3000/users', {
		...
		body: JSON.stringify ({
			...
			"passInput": Sha256.hash ( passInput.value ),
...
function chat() {
	...
	let updateChat = async function () {
		...
		if ( lastUpdate && updated.data === lastUpdate.data && 
			...
			await Promise.all ( [ getData ( "users" ).then ( x => users = x ) , 
		...
		initChat ()
		lastUpdate = { data: updated.data, time: updated.time }
			...
			chat.appendChild (
				( function () {
					...
					ava.title = ` ${user.name} ${user.lastName}`
										//appElem ( 'span', cont ).innerText = ` ${user.name} ${user.lastName}`
